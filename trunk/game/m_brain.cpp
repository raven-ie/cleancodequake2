...
{
	Sound (self, CHAN_AUTO, sound_idle3, 1, ATTN_IDLE, 0);
...
{
	Sound (self, CHAN_BODY, sound_melee1);
}
...
void brain_hit_right (edict_t *self)
{
	...
	if (fire_hit (self, aim, (15 + (rand() %5)), 40))
		Sound (self, CHAN_WEAPON, sound_melee3);
}

void brain_swing_left (edict_t *self)
{
	Sound (self, CHAN_BODY, sound_melee2);
}
...
void brain_chest_open (edict_t *self)
{
	...
	Sound (self, CHAN_BODY, sound_chest_open);
}
...
void brain_tentacle_attack (edict_t *self)
{
	...
	if (fire_hit (self, aim, (10 + (rand() %5)), -600) && skill->Integer() > 0)
		self->spawnflags |= 65536;
	Sound (self, CHAN_WEAPON, sound_tentacles_retract);
}
