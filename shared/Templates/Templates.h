...
template<>
inline sint32 Min(const sint32 &A, const sint32 &B)
{
	return (A<=B) ? A : B;
}
...
template<>
inline sint32 Max(const sint32 &A, const sint32 &B)
{
	return (A>=B) ? A : B;
}
...
template<>
inline sint32 Clamp(const sint32 &V, const sint32 &L, const sint32 &H)
{
	return (V<L) ? L : (V>H) ? H : V;
}

template<typename TType>
inline TType Bound(const TType &V, const TType &L, const TType &H)
{
	return (V>=H) ? V : (L<V) ? V : (L>H) ? H : L;
}
template<>
inline float Bound(const float &V, const float &L, const float &H)
{
	return (V>=H) ? V : (L<V) ? V : (L>H) ? H : L;
}
template<>
inline sint32 Bound(const sint32 &V, const sint32 &L, const sint32 &H)
{
	return (V>=H) ? V : (L<V) ? V : (L>H) ? H : L;
}
...
template<>
inline bool IsPowOfTwo(const sint32 &Value)
{
	return (bool)(Value > 0 && (Value & (Value-1)) == 0);
}
template<>
