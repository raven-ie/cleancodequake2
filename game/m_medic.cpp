...
void medic_idle (edict_t *self)
{
	...
	Sound (self, CHAN_VOICE, sound_idle1, 1, ATTN_IDLE, 0);
...
void medic_search (edict_t *self)
{
	...
	Sound (self, CHAN_VOICE, sound_search, 1, ATTN_IDLE, 0);
...
{
	Sound (self, CHAN_VOICE, sound_sight);
}
...
void medic_pain (edict_t *self, edict_t *other, float kick, int damage)
{
	...
	if (random() < 0.5)
	{
		...
		Sound (self, CHAN_VOICE, sound_pain1);
	}
	else
	{
		self->monsterinfo.currentmove = &medic_move_pain2;
		Sound (self, CHAN_VOICE, sound_pain2);
	}
}
...
// regular death
	Sound (self, CHAN_VOICE, sound_die);
	self->deadflag = DEAD_DEAD;
...
{
	Sound (self, CHAN_WEAPON, sound_hook_launch);
}
...
void medic_cable_attack (edict_t *self)
{
	...
	tr.Trace (start, self->enemy->s.origin, self, CONTENTS_MASK_SHOT);
	if (tr.fraction != 1.0 && tr.ent != self->enemy)
		return;

	if (self->s.frame == FRAME_attack43)
	{
		Sound (self->enemy, CHAN_AUTO, sound_hook_hit);
	...
	else
	{
		if (self->s.frame == FRAME_attack44)
			Sound (self, CHAN_WEAPON, sound_hook_heal);
	}
	...
	TempEnts.Trails.FleshCable (start, end, self - g_edicts);
}

void medic_hook_retract (edict_t *self)
{
	Sound (self, CHAN_WEAPON, sound_hook_retract);
	self->enemy->monsterinfo.aiflags &= ~AI_RESURRECTING;
}
