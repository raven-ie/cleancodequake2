...
bool M_CheckAttack (edict_t *self)
{
	...
	// see if any entities are in the way of the shot
		...
		tr.Trace (spot1, spot2, self, CONTENTS_SOLID|CONTENTS_MONSTER|CONTENTS_SLIME|CONTENTS_LAVA|CONTENTS_WINDOW);
...
//		gi.dprintf("checking for course correction\n");
		...
		tr.Trace (self->s.origin, self->mins, self->maxs, self->monsterinfo.last_sighting, self, CONTENTS_MASK_PLAYERSOLID);
		if (tr.fraction < 1)
		{
			...
			tr.Trace(self->s.origin, self->mins, self->maxs, left_target, self, CONTENTS_MASK_PLAYERSOLID);
			left = tr.fraction;

			Vec3Set (v, d2, 16, 0);
			G_ProjectSource (self->s.origin, v, v_forward, v_right, right_target);
			tr.Trace(self->s.origin, self->mins, self->maxs, right_target, self, CONTENTS_MASK_PLAYERSOLID);
			right = tr.fraction;
