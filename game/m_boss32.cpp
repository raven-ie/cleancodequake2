...
void makron_taunt (edict_t *self)
{
	...
	r=random();
	if (r <= 0.3)
		Sound (self, CHAN_AUTO, sound_taunt1);
	else if (r <= 0.6)
		Sound (self, CHAN_AUTO, sound_taunt2);
	else
		Sound (self, CHAN_AUTO, sound_taunt3);
}
...
{
	Sound (self, CHAN_AUTO, sound_hit, 1, ATTN_NONE,0);
}

void makron_popup (edict_t *self)
{
	Sound (self, CHAN_BODY, sound_popup, 1, ATTN_NONE,0);
}

void makron_step_left (edict_t *self)
{
	Sound (self, CHAN_BODY, sound_step_left, 1, ATTN_NORM,0);
}

void makron_step_right (edict_t *self)
{
	Sound (self, CHAN_BODY, sound_step_right, 1, ATTN_NORM,0);
}

void makron_brainsplorch (edict_t *self)
{
	Sound (self, CHAN_VOICE, sound_brainsplorch, 1, ATTN_NORM,0);
}

void makron_prerailgun (edict_t *self)
{
	Sound (self, CHAN_WEAPON, sound_prerailgun, 1, ATTN_NORM,0);
}
...
void makronBFG (edict_t *self)
{
	...
	Sound (self, CHAN_VOICE, sound_attack_bfg);
...
void makron_pain (edict_t *self, edict_t *other, float kick, int damage)
{
	...
	if (damage <= 40)
	{
		Sound (self, CHAN_VOICE, sound_pain4, 1, ATTN_NONE,0);
		self->monsterinfo.currentmove = &makron_move_pain4;
	}
	else if (damage <= 110)
	{
		Sound (self, CHAN_VOICE, sound_pain5, 1, ATTN_NONE,0);
	...
	else
	{
		if (damage <= 150)
			if (random() <= 0.45)
			{
				Sound (self, CHAN_VOICE, sound_pain6, 1, ATTN_NONE,0);
...
// regular death
	Sound (self, CHAN_VOICE, sound_death);
	self->deadflag = DEAD_DEAD;
...
bool Makron_CheckAttack (edict_t *self)
{
	...
	// see if any entities are in the way of the shot
		...
		tr.Trace (spot1, spot2, self, CONTENTS_SOLID|CONTENTS_MONSTER|CONTENTS_SLIME|CONTENTS_LAVA);
