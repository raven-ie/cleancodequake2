...
void gib_touch (edict_t *self, edict_t *other, plane_t *plane, cmBspSurface_t *surf)
{
	...
	if (plane)
	{
		Sound (self, CHAN_VOICE, gi.soundindex ("misc/fhit3.wav"));
...
{
	TempEnts.Explosions.GrenadeExplosion (self->s.origin);
	G_FreeEdict (self);
}
...
void commander_body_think (edict_t *self)
{
	...
	if (self->s.frame == 22)
		Sound (self, CHAN_BODY, gi.soundindex ("tank/thud.wav"));
}
...
void commander_body_use (edict_t *self, edict_t *other, edict_t *activator)
{
	...
	self->nextthink = level.time + FRAMETIME;
	Sound (self, CHAN_BODY, gi.soundindex ("tank/pain.wav"));
}
