...
{
	Sound (self, CHAN_BODY, sound_step);
}

void tank_thud (edict_t *self)
{
	Sound (self, CHAN_BODY, sound_thud);
}

void tank_windup (edict_t *self)
{
	Sound (self, CHAN_WEAPON, sound_windup);
}
...
void tank_pain (edict_t *self, edict_t *other, float kick, int damage)
{
	...
	Sound (self, CHAN_VOICE, sound_pain);
...
{
	Sound (self, CHAN_WEAPON, sound_strike);
}	
...
void tank_reattack_blaster (edict_t *self)
{
	if (skill->Integer() >= 2 && visible (self, self->enemy) && self->enemy->health > 0 && random() <= 0.6)
	{
...
void tank_refire_rocket (edict_t *self)
{
	...
	if ( skill->Integer() >= 2 && self->enemy->health > 0 && visible(self, self->enemy) && random() <= 0.4)
	{
