...
//	Sound (self, CHAN_VOICE, actor.sound_pain);
...
// check for gib
	if (self->health <= -80)
	{
//		Sound (self, CHAN_VOICE, actor.sound_gib);
		for (n= 0; n < 2; n++)
...
// regular death
//	Sound (self, CHAN_VOICE, actor.sound_die);
	self->deadflag = DEAD_DEAD;
...
void target_actor_touch (edict_t *self, edict_t *other, plane_t *plane, cmBspSurface_t *surf)
{
	...
	if (self->spawnflags & 1)		//jump
	{
		...
		if (other->groundentity)
		{
			...
			Sound(other, CHAN_VOICE, gi.soundindex("player/male/jump1.wav"));
		}
	}
