...
void ChickMoan (edict_t *self)
{
	if (random() < 0.5)
		...
		Sound (self, CHAN_VOICE, sound_idle2, 1, ATTN_IDLE, 0);
}
...
void chick_pain (edict_t *self, edict_t *other, float kick, int damage)
{
	...
	if (r < 0.33)
		...
		Sound (self, CHAN_VOICE, sound_pain3);
...
// regular death
	...
	if (n == 0)
	{
		...
		Sound (self, CHAN_VOICE, sound_death1);
	}
	else
	{
		self->monsterinfo.currentmove = &chick_move_death2;
		Sound (self, CHAN_VOICE, sound_death2);
	}
}
...
void ChickSlash (edict_t *self)
{
	...
	Sound (self, CHAN_WEAPON, sound_melee_swing);
...
{
	Sound (self, CHAN_VOICE, sound_missile_prelaunch);
}

void ChickReload (edict_t *self)
{
	Sound (self, CHAN_VOICE, sound_missile_reload);
}
